...
def astar(graph, nodes, start, end):
    ...
    open_set = []
    heapq.heappush(open_set, (haversine(*nodes[start], *nodes[end]), start))
    ...
    while open_set:
        ...
        for neighbor, weight in graph.get(current, []):
            ...
            if tentative_g < g_score.get(neighbor, float("inf")):
                ...
                f_new = tentative_g + haversine(*nodes[neighbor], *nodes[end])
...
def find_shortest_path(start_lat, start_lon, end_lat, end_lon, nodes, graph):
    ...
    coords = [{"lat": nodes[nid][0], "lon": nodes[nid][1]} for nid in path]
