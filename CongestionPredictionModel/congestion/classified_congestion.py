...
class GoongRoadGeometryFetcher:
    ...
    @staticmethod
    def _safe_segment_name(raw_name, seg_id):
        # Handles pandas NaN, None, empty string, and "nan" text
        try:
            import pandas as pd
        ...
        return str(seg_id) if (s == "" or s.lower() == "nan") else s
        ...
        for idx, row in gps_segments.iterrows():
            ...
            if route_data['success']:
                ...
                feature = {
                    ...
                    'properties': {
                        'segment_id': seg_id,
                        'segment_name': self._safe_segment_name(row['segment_name'], seg_id),
                        'lat': lat,
                        'lon': lon,
...
class CongestionClassifier:
    ...
    def update_geojson_with_congestion(self, classified_csv='segment_congestion_classified.csv',
        ...
        df["time_window"] = pd.to_datetime(df["time_window"], format="mixed", errors="coerce")
        ...
        for feature in geojson['features']:
            ...
            # Force segment_name to be valid (no NaN)
            raw_name = feature['properties'].get('segment_name', None)
            feature['properties']['segment_name'] = GoongRoadGeometryFetcher._safe_segment_name(raw_name, seg_id)
