# Run: python multi_camera_loop.py --cameras-file ./cameras.json --interval 1800 --max-workers 6 --output-dir ./camera_frames --max-keep 3
import argparse
...
class HCMCCameraCrawler:
    def __init__(
        ...
        self.max_keep = int(max_keep) if max_keep is not None else 3
    ...
    def save_image(self, image_data):
        ...
        # Get GPS data from camera_locations if available
        loc = self.camera_locations.get(self.camera_id, {})

        metadata = {
            "camera_id": self.camera_id,
            "camera_name": self.camera_name,
            "timestamp": timestamp,
            "timestamp_iso": utc_now().isoformat().replace("+00:00", "Z"),
            "hash": image_hash,
            ...
            "location_confidence": loc.get('confidence'),
            "camera_code": loc.get('camera_code'),
            "bearing": None,
            "lane_schema": None
        }
    ...
    def crawl_once(self):
        ...
        if saved is not None:
            self.cleanup_old_images(max_keep=self.max_keep)  # keep only newest N per camera
    ...
    def cleanup_old_images(self, max_keep=None):
        """
...
class MultiCameraLoop:
    def __init__(self, camera_entries, interval_seconds=120, max_workers=16, output_dir="./camera_frames", max_keep=3, camera_locations=None):
        ...
        for e in camera_entries:
            ...
            if not cid:
                continue
            self.crawlers.append(HCMCCameraCrawler(
                str(cid),
                str(name),
    ...
    def run(self):
        ...
        print(f"Starting loop: {total_cams} cameras, interval={self.interval}s, workers={self.max_workers}, max_keep={self.max_keep}")
        # Housekeeping: ensure folders start clean (e.g., from previous runs)
        for c in self.crawlers:
            try:
                c.cleanup_old_images(max_keep=self.max_keep)
            except Exception as e:
                print(f"⚠️ Cleanup failed for {c.camera_id}: {e}")
        cycle = 0
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as ex:
                while not self._stop.is_set():
                    ...
                    # Housekeeping: keep only newest N images per camera
                    for c in self.crawlers:
                        try:
                            c.cleanup_old_images(max_keep=self.max_keep)
                        except Exception as e:
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=3,
        help="Keep only the newest N images per camera (housekeeping cleanup).",
    )
    args = p.parse_args()
...
def main():
    ...
    # Load camera locations if available
    camera_locations = {}
    locations_file = Path("camera_locations.json")
    if locations_file.exists():
        try:
            camera_locations = json.loads(locations_file.read_text(encoding='utf-8'))
            print(f"✓ Loaded GPS data for {len(camera_locations)} cameras")
        except Exception as exc:
            print(f"Warning: Could not load camera_locations.json: {exc}")
